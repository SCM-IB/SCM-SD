...
def compute_chamfer_distance(points_pred, points_gt, max_dist=1.0):
    ...
    if points_pred.shape[0] > MAX_POINTS:
        np.random.seed(int(time.time()))  # Fix random seed
...
def get_vgg_input_imgs(images: np.ndarray):
    ...
    for image in images:
        ...
        new_width = 518
        new_height = round(height * (new_width / width) / 14) * 14 # Scale image to new width new_width = 518，while maintaining aspect ratio，and let the new height be 14 an integer multiple of
...
def calculate_auc(r_error, t_error, max_threshold=30):
    """
    Calculate the Area Under the Curve (AUC) for pose estimation errors.
    
    Args:
        r_error: numpy array representing rotation error values (degrees)
        t_error: numpy array representing translation error values (degrees)
...
def compute_relative_pose_errors(poses_est, poses_gt, num_frames):
    """
    Compute rotation and translation errors between predicted and ground truth poses.
    This follows Pi3's relative pose metric implementation and expects world-to-camera
    SE(3) matrices as input.
    
    Args:
        poses_est: Predicted camera-to-world transformations, shape (N, 4, 4)
        poses_gt: Ground truth camera-to-world transformations, shape (N, 4, 4)
        num_frames: Number of frames (N)
    
    Returns:
        Rotation and translation angle errors in degrees
    """
    # Build pair indices for relative pose computation
    pair_idx_i1, pair_idx_i2 = [], []
    for i in range(num_frames):
        for j in range(i + 1, num_frames):
            pair_idx_i1.append(i)
            pair_idx_i2.append(j)
    
    pair_idx_i1 = np.array(pair_idx_i1)
    pair_idx_i2 = np.array(pair_idx_i2)
    
    # Pi3 computes relative poses in w2c convention as T_j * inv(T_i).
    relative_pose_gt = poses_gt[pair_idx_i2] @ np.linalg.inv(poses_gt[pair_idx_i1])
    relative_pose_pred = poses_est[pair_idx_i2] @ np.linalg.inv(poses_est[pair_idx_i1])
    
    # Compute rotation error
    rel_rangle_deg = rotation_angle_between_matrices(
        relative_pose_gt[:, :3, :3], relative_pose_pred[:, :3, :3]
    )
    
    # Compute translation error
    rel_tangle_deg = translation_angle_between_vectors(
        relative_pose_gt[:, :3, 3], relative_pose_pred[:, :3, 3]
    )
...
def rotation_angle_between_matrices(R_gt, R_pred):
    """
    Calculate rotation angle error between ground truth and predicted rotation matrices.
    
    Args:
        R_gt: Ground truth rotation matrices, shape (N, 3, 3)
        R_pred: Predicted rotation matrices, shape (N, 3, 3)
    
    Returns:
        Rotation angle error in degrees
    """
    R_gt_t = torch.as_tensor(R_gt, dtype=torch.float32)
    R_pred_t = torch.as_tensor(R_pred, dtype=torch.float32)
    q_pred = mat_to_quat(R_pred_t)
    q_gt = mat_to_quat(R_gt_t)
    loss_q = (1 - (q_pred * q_gt).sum(dim=1) ** 2).clamp(min=1e-15)
    err_q = torch.arccos(1 - 2 * loss_q)
    return (err_q * 180.0 / np.pi).cpu().numpy()


def translation_angle_between_vectors(t_gt, t_pred):
    """
    Calculate translation angle error between ground truth and predicted translation vectors.
    
    Args:
        t_gt: Ground truth translation vectors, shape (N, 3)
        t_pred: Predicted translation vectors, shape (N, 3)
    
    Returns:
        Translation angle error in degrees
    """
    t_gt_t = torch.as_tensor(t_gt, dtype=torch.float32)
    t_pred_t = torch.as_tensor(t_pred, dtype=torch.float32)

    t_pred_t = t_pred_t / (torch.norm(t_pred_t, dim=1, keepdim=True) + 1e-15)
    t_gt_t = t_gt_t / (torch.norm(t_gt_t, dim=1, keepdim=True) + 1e-15)

    loss_t = torch.clamp_min(1.0 - torch.sum(t_pred_t * t_gt_t, dim=1) ** 2, 1e-15)
    err_t = torch.acos(torch.sqrt(1 - loss_t))
    err_t[torch.isnan(err_t) | torch.isinf(err_t)] = 1e6

    angle_deg = err_t * 180.0 / np.pi
    angle_deg = torch.minimum(angle_deg, (180.0 - angle_deg).abs())
...
def build_frame_selection(
    ...
    # When the number of valid frames available exceeds the number of frames required，Select a specified number of frames by uniformly sampling 。
