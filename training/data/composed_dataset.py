...
class ComposedDataset(Dataset, ABC):
    ...
    """
    """
    Combine multiple underlying datasets and apply common configurations。
    This dataset provides a flexible way，
    Ability to combine multiple underlying data sets，Also apply shared enhanced processing、Trace generation and other processing steps。
    It is able to handle image normalization、Tensor transformations and other image sequence preparation required for training computer vision models。
    """
    def __init__(self, dataset_configs: dict, common_config: dict, **kwargs):
        ...
        Args:
            ...
            **kwargs: Additional arguments (unused).
            Refer to dataset & common_config in both two .yaml files
        """
        base_dataset_list = []
    ...
    def __getitem__(self, idx_tuple):
        ...
        """
        """
        Retrieve a data sample from the data set（sequence）。
        Load raw data，convert it to PyTorch Tensor，Apply enhancement actions
        """
        ...
        """   
        # point to TupleConcatDataset().__getitem__() Return BaseDataset().__getitem__() 
            referring to VKittiDataset().get_data()
        """
        ...
        # Infrared images (student input)
        infrared_images = torch.from_numpy(np.stack(batch["infrared_images"]).astype(np.float32)).contiguous()
        infrared_images = infrared_images.permute(0,3,1,2).to(torch.get_default_dtype()).div(255)
        
        # Visible images (teacher input for knowledge distillation)
        visible_images = torch.from_numpy(np.stack(batch["images"]).astype(np.float32)).contiguous()
        visible_images = visible_images.permute(0,3,1,2).to(torch.get_default_dtype()).div(255)
        ...
        if self.training and self.image_aug is not None:
            if self.cojitter and random.random() > self.cojitter_ratio:
                ...
                infrared_images = self.image_aug(infrared_images)
            else:
                # Apply different color jittering to each frame individually
                for aug_img_idx in range(len(infrared_images)):
                    infrared_images[aug_img_idx] = self.image_aug(infrared_images[aug_img_idx])
        ...
        sample = {
            ...
            "frame_num": batch.get("frame_num", len(infrared_images)),
        }
...
class TupleConcatDataset(ConcatDataset):
    ...
    def __init__(self, datasets, common_config):
        ...
        # if true，then the input index is ignored，and randomly sampled on all datasets
        # This provides an alternative to data loader shuffling for large datasets
