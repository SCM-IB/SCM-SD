...
def crop_image_depth_and_intrinsic_by_pp(
        ir_image, image, depth_map, intrinsic, target_shape, track=None, filepath=None, strict=False
):
    """
    ...
    """
    original_size = np.array(ir_image.shape)
    ...
    # Perform the crop
    ir_image = ir_image[start_x:end_x, start_y:end_y, :]
    ...
    if strict:
        if (ir_image.shape[:2] != target_shape).any():
            print(f"{filepath} does not meet the target shape")
            current_h, current_w = ir_image.shape[:2]
    ...
    return ir_image, image, depth_map, intrinsic, track
...
def resize_image_depth_and_intrinsic(
    ...
    ir_image = Image.fromarray(ir_image)
    image = Image.fromarray(image)
    input_resolution = np.array(ir_image.size)
    output_resolution = np.floor(input_resolution * max_resize_scale).astype(int)
    ir_image = ir_image.resize(tuple(output_resolution), resample=lanczos if max_resize_scale < 1 else bicubic)
    ...
    actual_size = np.array(ir_image.shape[:2])
    ...
    assert ir_image.shape[:2] == depth_map.shape[:2]
...
def rotate_90_degrees(
    ir_image, image, depth_map, extri_opencv, intri_opencv, clockwise=True, track=None
):
    """
    ...
    """
    image_height, image_width = ir_image.shape[:2]

    # Rotate the image and depth map
    rotated_ir_image, rotated_image, rotated_depth_map = rotate_image_and_depth_rot90(ir_image, image, depth_map, clockwise)
...
def rotate_image_and_depth_rot90(ir_image, image, depth_map, clockwise):
    """
    ...
    if clockwise:
        rotated_ir_image = np.transpose(ir_image, (1, 0, 2))  # Transpose height and width
        rotated_ir_image = np.flip(rotated_ir_image, axis=1)  # Flip horizontally
        ...
        rotated_ir_image = np.transpose(ir_image, (1, 0, 2))  # Transpose height and width
        rotated_ir_image = np.flip(rotated_ir_image, axis=0)  # Flip vertically
        rotated_image = np.transpose(image, (1, 0, 2))  # Transpose height and width
        rotated_image = np.flip(rotated_image, axis=0)  # Flip vertically
        if depth_map is not None:
            rotated_depth_map = np.transpose(depth_map, (1, 0))
            rotated_depth_map = np.flip(rotated_depth_map, axis=0)
    return np.copy(rotated_ir_image), np.copy(rotated_image), np.copy(rotated_depth_map)
...
# Read camera parameters(ex and in) for a specific scene in COOLMAP-style dataset
import struct
def read_extrinsics_test(scene_path):
    """
    readCOLMAPofimages.bindocument
    return: imageslist
    """
    images = []
    if os.path.exists(os.path.join(scene_path, "colmap/sparse/0", "images.bin")):
        cameras_extrinsic_file = os.path.join(scene_path, "colmap/sparse/0", "images.bin")
        with open(cameras_extrinsic_file, "rb") as fid:
            ...
            for _ in range(num_reg_images):
                ...
                print(f"image_id: {image_id}, camera_id: {camera_id}")

                image_name = ""
                current_char = read_next_bytes(fid, 1, "c")[0]
                while current_char != b"\x00":   # End on null character
...
def read_camera_extrinsics(scene_path):
    if os.path.exists(os.path.join(scene_path, "colmap/sparse/0", "images.bin")):
        cameras_extrinsic_file = os.path.join(scene_path, "colmap/sparse/0", "images.bin")
        images = []
        with open(cameras_extrinsic_file, "rb") as fid:
            ...
            for _ in range(num_reg_images):
                ...
                images.append(
                    ...
                    camera_id=camera_id, name=image_name))
        return images
    elif os.path.exists(os.path.join(scene_path, "colmap/sparse/0", "images.txt")):
        cameras_extrinsic_file = os.path.join(scene_path, "colmap/sparse/0", "images.txt")
        images = []
        ...
        raise ValueError(f"There is no images.bin/.txt for camera_extrinsics in {scene_path}")
BaseImage = collections.namedtuple(
    "BaseImage", ["id", "qvec", "tvec", "camera_id", "name"])

def read_camera_intrinsics(scene_path):
    if os.path.exists(os.path.join(scene_path, "colmap/sparse/0", "cameras.bin")):
        cameras_intrinsic_file = os.path.join(scene_path, "colmap/sparse/0", "cameras.bin")
        cameras = {}
        with open(cameras_intrinsic_file, "rb") as fid:
            ...
            for _ in range(num_cameras):
                ...
                #print(f"Camera {camera_id} has params: {params}")
            assert len(cameras) == num_cameras
        return cameras
    elif os.path.exists(os.path.join(scene_path, "colmap/sparse/0", "cameras.txt")):
        cameras_intrinsic_file = os.path.join(scene_path, "colmap/sparse/0", "cameras.txt")
        cameras = {}
        ...
        raise ValueError(f"There is no cameras.bin/.txt for camera_intrinsic in {scene_path}")
Camera = collections.namedtuple(
    "Camera", ["id", "model", "width", "height", "params"])


def colmap2opencv(intr: Camera, extr: BaseImage):
    model = intr.model
    params = intr.params
    width = intr.width
    height = intr.height
    ...
    extri_opencv = np.hstack((R, tvec.reshape(3, 1)))
    #extri_opencv = np.vstack((extri_opencv, [0, 0, 0, 1]))
    #print(f"dataset_utils::::::extri_opencv.shape: {extri_opencv.shape}")
