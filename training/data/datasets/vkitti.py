...
class VKittiDataset(BaseDataset):
    def __init__(
        ...
        if osp.exists(txt_path):
            ...
            sequence_list = [file_path.split(self.VKitti_DIR)[-1].lstrip('/') for file_path in sequence_list] # [*/*/*/rgb/*, ...]
    ...
    def get_data(
        ...
        Args:
            ...
            aspect_ratio (float): Aspect ratio for image processing. Actually it's high/Width ratio
        ...
        # Load camera parameters Load camera parameters
        try:
        ...
        if ids is None:
            #  from0 arrive num_images - 1 Randomly selected within the range of integers img_per_seq image index（ID），Whether to allow duplication by self.allow_duplicate_img control
        ...
        for image_idx in ids:
            ...
            intri_opencv[0, 0] = camera_intrinsic[image_idx][-4]  # fx
            intri_opencv[1, 1] = camera_intrinsic[image_idx][-3]  # fy
            intri_opencv[0, 2] = camera_intrinsic[image_idx][-2]  # cx
            intri_opencv[1, 2] = camera_intrinsic[image_idx][-1]  # cy
