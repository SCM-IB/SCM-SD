...
"""
When DynamicTorchDataset().get_loader(epoch)
1、set up epoch：
    sampler.set_epoch(epoch) → Ensure distributed disruption is consistent；
2、DataLoader start up，use DynamicBatchSamplerIterate；
3、DynamicBatchSampler.__iter__()：
    random sampling image_num and aspect_ratio；
    call sampler.update_parameters(...)；
    Dynamically calculate current batch_size = floor(max_img_per_gpu / image_num)；
    from sampler(DynamicDistributedSampler) get batch_size indivual (idx, img_num, aspect_ratio) tuple；
    yield The batch of triples；
DataloaderAutomatic execution：
4、ComposedDataset.__getitem__((idx, img_num, aspect_ratio))：
    use img_num and ar Dynamically generated/Crop/Combined samples（Such as multi-view image synthesis）；
5、pass collate_fn combined into batch Tensor，Feed model。
"""
...
class DynamicTorchDataset(ABC):
    def __init__(
        ...
        # Instantiate the dataset (i.e data.composed_dataset.ComposedDataset)
        ...
        self.aspect_ratio_range = common_config.augs.aspects  # e.g., [0.33, 1.0]
...
class DynamicBatchSampler(Sampler):
    ...
    def __iter__(self):
        ...
        """
        sampler_iterator = iter(self.sampler) # Instantiate
...
class DynamicDistributedSampler:
    """
    A sampler that supports both distributed and non-distributed training.
    When distributed training is available, it extends PyTorch's DistributedSampler.
    Otherwise, it uses RandomSampler with dynamic aspect_ratio and image_num
