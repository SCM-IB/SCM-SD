...
class DurationMeter:
    """
    Used to track and format time durations（duration），For example, how many seconds does an operation take?。
    """
    ...
    def update(self, val):
        # Directly overwrite the current value。Suitable for recording single time consumption（For example, the time of a certain forward propagation）
        self.val = val

    def add(self, val):
        # Accumulated time value。Applicable to the total time taken for multiple operations（as whole epoch All in batch The total data loading time of）
        self.val += val

    def __str__(self):
        # Returns a human-readable time string
        # Usage: timer = DurationMeter("time is",device="cuda:0")
        #        start = time.time()
        #        timer.update(time.time() - start)    print(timer)
...
class ProgressMeter:
    """
    It is designed to track and display the progress of a process (e.g., during training or evaluation) over batches.
    """
    ...
    def display(self, batch):
        # This method is responsible for constructing and logging the progress message for the current batch.
    ...
    def _get_batch_fmtstr(self, num_batches):
        # It calculates the number of digits needed to represent the total number of batches
        # and uses that to construct a formatted string
        # e.g., for 100 batches, it would output something like [01/100], [10/100], or [100/100] depending on the batch number.
...
def model_summary(model: torch.nn.Module,
    ...
    # Separate counts for RGB (visible) and infrared branches for VGGTDualBranch
    rgb_total = 0
    rgb_trainable = 0
    infrared_total = 0
    infrared_trainable = 0

    if hasattr(model, 'visible_aggregator') and hasattr(model, 'infrared_aggregator'):
        # Count RGB (visible) branch parameters
        for name, param in model.named_parameters():
            if name.startswith('visible_'):
                rgb_total += param.numel()
                if param.requires_grad:
                    rgb_trainable += param.numel()
            elif name.startswith('infrared_'):
                infrared_total += param.numel()
                if param.requires_grad:
                    infrared_trainable += param.numel()
        ...
        print(prefix + '-'*80)
        print(prefix + f'RGB Branch : {pretty_int(rgb_total)} parameters')
        print(prefix + f'  trainable: {pretty_int(rgb_trainable)}')
        print(prefix + f'  frozen   : {pretty_int(rgb_total - rgb_trainable)}')
        print(prefix + f'Infrared Branch : {pretty_int(infrared_total)} parameters')
        print(prefix + f'  trainable: {pretty_int(infrared_trainable)}')
        print(prefix + f'  frozen   : {pretty_int(infrared_total - infrared_trainable)}')
        print(prefix + '='*80)
    else:
        # Original summary for other model types
        print(prefix + '='*60)
        print(prefix + f'Model type : {model.__class__.__name__}')
        print(prefix + f'Total      : {pretty_int(total)} parameters')
        print(prefix + f'  trainable: {pretty_int(trainable)}')
