...
def setup_logging(
    ...
    """
    """
    setup_logging function is a function for Configuration Python Logging system Utility tools，Designed for Distributed training environment（Ruduo GPU）design。Its core goal is：
        exist main process（rank=0） Output detailed log（like INFO）；
        exist other processes（rank>0） Only output high priority logs（like ERROR）；
        Optionally write logs to a file（The default is only in rank=0 Write，unless all_ranks=True）；
        Avoid log duplication（Clean up existing handler）
    """
