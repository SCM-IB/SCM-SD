...
def construct_optimizer(model: nn.Module,
                        optimizer_conf: Any,
                        options_conf: Union[Mapping[str, List], None] = None, #Parameter group scheduling configuration，Allows applying different optimization options to different parameters（like backbone Use a low learning rate，head Use high learning rate）
                        param_group_modifiers_conf: Union[List, None] = None, #User-defined parameter group modification logic（Advanced usage
    ...
    *No* allowlist handling – we always optimize *all* model parameters.
    """
    """
     Hydra Configuration flexibility to build a schedule with optional parameter groups（Such as learning rate grouping）The optimizer，and returns an encapsulated OptimizerWrapper object
     It supports two modes：
        Simple mode：All parameters use the same optimizer configuration（No grouping）；
        Advanced mode：Different parameters according to configuration（Such as different layers、module）Apply different optimizer options（such as different learning rates、Weight decay, etc.），
                Even supports custom modifiers（modifiers）。
    """
